...
ULONG EVNTAPI EventWrite(REGHANDLE reghandle, PCEVENT_DESCRIPTOR evdesc, ULONG count, PEVENT_DATA_DESCRIPTOR evdata)
{
...
/// @return ERROR_SUCCESS if the operation was successful.
ULONG EVNTAPI EventRegister(LPCGUID provider, PENABLECALLBACK enablecb, PVOID ctx, PREGHANDLE reghandle)
{
...
/// @param count The maximum number of characters to write to the buffer, including zero byte.
/// @param format A NULL-terminated string following printf format specified rules.
/// @param args Substitution arguments for the format string.
